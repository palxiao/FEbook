...
export default (vue: typeof page) => {
  ...
  vue.setup = (...args) => {
    const state = (_setup && _setup(...args)) as State // 
...
```js
const len = state.form_6_child.length
state.form_6_child[len - 1].send = () => {
  console.log('formData 提交数据：', state.formData.value['form_6'])
  // http(state.form_1.api.save, 'POST', state.formData.value['form_15']).then((res) => {
  //   console.log(res)
  // })
}

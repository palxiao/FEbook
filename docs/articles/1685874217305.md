...
不可否认，低代码是需要有一定学习成本的，但同样可能降低成本。举个例子，例如公司的微信小程序是基于 uniapp 开发，又使用了 uview 等框架，开发语言为 vue3，那么我除了要会 vue3 以外，还需要同时阅读小程序文档、uniapp文档、uview框架文档，而低代码开发则只依赖于低代码文档，低代码不支持的才需要额外工作量。

### 低代码与零代码

零代码：指的是非技术人员通过拖拉拽等方式，无需编写代码，即可快速搭建各种应用。这种无代码方式只适合单点场景的应用，扩展性很差，主要的设计考虑在易用性层面。

低代码：主要面向开发人员（特别是对前端不熟悉的开发），通过自动代码生成和可视化编程，只需少量代码，即可快速搭建页面。低代码既要考虑一定的易用性，也要充分保证灵活性、可扩展性(代码增强)，这种混合开发能够基本满足具有较复杂场景交互的应用。

总的来说，低代码的核心即是把 90% 的工作交给程序去完成，而业务开发人员则负责“**走完最后的一公里路**”。
...
上面的代码是非常容易理解的，但是其中的 `_customFn` 方法可能会让你感到疑惑，其实如果你稍微使用一下平台层就会发现，这个方法是可以配置的，如果不配置的话默认即是 `_customFn`。

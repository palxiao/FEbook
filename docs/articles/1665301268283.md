...
这是一个纯前端的项目，不需要开发后端，没有数据库，只需要把照片丢进去，Git提交一下站点就出来了，就很简单。为什么用**GithubPages**部署？因为不用买服务器。为什么不用OSS做图床，因为要钱，一张照片几兆，但好在完全**免费**，全是白嫖的艺术，~~甚至看完文章不点赞不收藏不评论白嫖我都可以~~，这篇文章中我将介绍整个项目的开发历程，看完你将收获：

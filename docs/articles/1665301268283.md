...
经过0.1024秒的思索，我马上发现了问题所在，上面的代码仅仅只是把图片按左右的顺序往下排列，而图片高度是不一样的，就会出现尾部瘸腿的现象，解决的办法也很简单，**找出最短的那一列**插入图片即可，我们已经将高度都存在了`columnHeights`这个数组中，通过往`Math.min()`传入**解构数组**得到最小值，再用`indexOf`得到下标，就可以知道下一张图片该插入哪一列了，稍微修改一下代码：

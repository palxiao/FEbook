...
上面推导过程我都是在纸上完成的，回到代码中，我们可以使用**递归**来操作图片数组，得到一组计算好宽高的新数组，这里我设计了一个工厂函数`factory`以及计算函数`calculate`，计算函数核心就是利用上面的公式求图片高度，而工厂函数则是递归取出图片去执行计算，如果高度在我们设置的**阈值**之内那么就将这些图片"**打包**"返回，在`handleList`函数中拼成一个二维数组，最后**拍平数组**：

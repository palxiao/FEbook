...
为什么要解码图片呢？如果你经常接触移动端H5开发，应该会碰到过**图像翻转问题**，因为手机照片通常会以原始的**拍摄方向**展示，可能你在手机上看着没什么问题，但是直接链接到网页上显示就会发现图像翻转了，所以我们需要手动翻转图像到正确位置并重新编码图像。
...
通过 **images** 重新编码的照片，元数据也会被抹除，所以我们可以配合前面 **exif** 检查照片是否存在 **GPS** 对象

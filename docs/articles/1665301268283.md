...
好了，现在把照片放在**resources**目录，创建一个`index.js`文件使用**NodeJs**来遍历**resources**，我们要获得路径名称、图片大小等信息(使用 [image-size](https://www.npmjs.com/package/image-size) 这个库可以快速获取图片大小)，然后输出成**JSON**文件保存，最后将图片复制到目录`view/public/`中，这样我们就可以开始编写前端项目了。

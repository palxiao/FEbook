...
这是一个纯前端的项目，不需要开发后端，没有数据库，只需要把照片丢进去，Git提交一下站点就出来了，就很简单。为什么用**GithubPages**部署？因为不用买服务器。为什么不用OSS啥的做图床，因为要钱，一张照片几兆，但好在完全**免费**，全是白嫖的艺术，~~甚至看完文章不点赞不收藏不评论白嫖我都可以~~，这篇文章中我将介绍整个项目的开发历程，看完你将收获：
...
好了，现在把照片放在**resources**目录，创建一个`index.js`文件使用**NodeJs**来遍历**resources**，我们要获得路径名称、图片大小等信息(使用 [image-size](https://www.npmjs.com/package/image-size) 这个库可以快速获取图片大小)，然后输出成**JSON**文件保存，最后将图片复制到目录`view/public/`中:

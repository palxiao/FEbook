...
但是懒加载我们使用另一种方式实现：[Intersection Observer](https://developer.mozilla.org/zh-CN/docs/Web/API/IntersectionObserver)，这是一个浏览器原生API，可以用于异步观察目标元素与其祖先元素或顶级文档视窗是否**交叉**的方法，简单讲就是可以监听一个元素是否出现在视窗当中，就这么简单粗暴，该API其实已经提出很久了，所以不需要担心兼容性问题。

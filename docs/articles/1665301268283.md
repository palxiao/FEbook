...
即使你使用手机拍照，并不关心照片拍摄时的设备与其它参数，你最起码也应该知道这张照片是在何时拍摄的，而且照片的**时间**也是我们排序图片的一个重要依据，而前面我们用**NodeJs**读取照片源文件时，通过`fs.stat`来获取文件的创建/修改时间其实并不能代表实际拍摄时间，此时就要通过 **Exif** (Exchangeable image file format)

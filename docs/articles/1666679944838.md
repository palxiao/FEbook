...
PC上的点击、移动，移动端的手势操作，都离不开DOM**事件监听**，例如鼠标移动事件对应 `mousemove`，移动端触摸移动则对应 `touchmove`，而本文将介绍如何仅通过**指针事件**进行多端统一的事件监听。在监听事件中我们可以通过 `event` 对象获取各种属性，例如常用的 `offsetX` 和 `offsetY` 相对偏移量，clientX，clientY 标识的鼠标当前横坐标和纵坐标

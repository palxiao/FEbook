...
这里我们使用 `Map` 数组对触摸点进行记录，通过这个实例你也可以看到 `Map` 在数组操作上有多么方便，直接调用 `api` 即可完成数组的增删改查，其中我们利用 `pointerId` 标识触摸点，当移动时根据其 `pointerId` 更新对应的数据，当触点抬起时删除点位：
...
现在我们有了对**触摸点判断**的基础，就可以开始实现**缩放**了，当双指按下时，我们需要记录下**两点间距离**作为初始值，此时我们双指在屏幕上滑动，**两点间距**不停发生变化，此时存在一个变化**比例** = `当前距离 / 初始距离`，该比例作为改变 `scale` 的系数就能得到新的缩放值。

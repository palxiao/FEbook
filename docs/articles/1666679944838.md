...
这时缩放感觉是没有问题了，但是每当往屏幕中的不同位置再次进行缩放操作时，图片会突然间闪动一下位置，到最后几乎不受控制。



效果演示如下，啊~ 如此丝滑，流下了两行热泪
...
可以看到 **vant** 并没有使用任何 `translate3d` 或 `scale3d` 属性，看来是真的有坑了。其实我们使用 `translate3d` 也是把第三个参数一直设置为`0`，这和 `translateZ(0)` 是等价的，但奇怪的是单独设置 `translateZ` 却没有引发问题。

...
以上仅是实现了缩放的处理，而**缩放原点**还在默认的图片中心，就和PC端一样，对于双指缩放来说，改变的只是两点间距离，无论双指间距如何改变，两点连成的**线段中心点**是不会变的，所以我们只要通过缩放的这两个点求出中心点坐标设置为原点坐标即可：
...
这就回到前面提到的，**原点位置突然改变**带来的偏移量引起了图片位置的突然闪动，那么这段偏移是如何产生的呢，我们画两张图看下，在原点变化的前后图像的坐标点发生了什么变化：



如图，原点为**O**时，我们取右下角的点设为**A**，图像放大2倍时**A**点变换到**B**点



我们在进行缩放时已知的变量有`scale`这个值，双指落下时我们手动改变了缩放原点，原点也是已知的，而图像原本的**A**点是不变的。
...
我们可以把**图像的偏移**抽象为图像某个**点位的偏移**，这样问题就变成了计算点坐标的距离：
...
答案是后两者都会使**移动端的图片变模糊**！起初我发现图片在手机上模糊的问题时，调试很久都没定位到源头，一筹莫展之际想起以前做图片预览都是使用 **vant**，那就看看它的源码吧，没想到很快就找到[相关代码](https://github.com/youzan/vant/blob/main/packages/vant/src/image-preview/ImagePreviewItem.tsx)：

金九银十送你一份前端超详细知识手册，看完一定会有收获！

花了大量时间整理的前端知识手册，内容由浅到深，主要是笔者对前端知识的复盘合集，干货满满，分享出来希望能一起交流进步，同样适合各个阶段的前端人学习和巩固，面试备战。另个人知识水平有限，如有纰漏，望不吝指正。
...
我们都知道前端体系领域广，知识又碎片化，因此我比较喜欢将零碎的知识点记录汇总起来，首先记录的所学所思可方便日后复习，在这个过程中也能不断深入强化理解，最终文章的目录会自然地形成一个知识脉络的树形图（对于我这种不会画思维导图的人来说，这种自下而上的方式更合适）。
...
> 常见的 window 对象属性：`document`、`location`、`navigator`、`screen`、`history`、`frames`
...
2. 响应式布局（**css 媒体查询**）
3. rem，%，vh vw等单位（可通过 **postcss** 插件自动转化 px 单位）
...
其它的方式缺点太多建议了解即可，其中 grid 是最强大的二维布局方式，但只是做居中布局还是一维的 flex 就够用了。
...
5. `clip-path: circle(0px);`：利用裁剪方式创建元素的可显示区域，区域外会隐藏，占据空间，不可选中
...
简单来说就是函数中嵌套函数（或函数中返回函数），作用是可以访问局部变量，缺点是容易发生**内存泄漏**（变量不会被自动回收）。
...
上面这种会了继承你就会用了，但要说真想搞懂继承，还得继续往下看，首先JS继承有哪些方法，摊开手掌✋往前一摆：
...
JS是单线程，包含了同步任务与异步任务，同步任务放入**调用栈**(主线程)中执行，异步任务会放入**消息队列**(TaskQueue)中，等待同步任务执行完毕再取出来执行，而此时如果**异步之中仍有异步**任务，则会继续放入消息队列中等待，这就是JS的**事件循环机制**(EventLoop)。
...
**宏任务**：
    ...
    定时器系列：setTimeOut、setInterval、requestAnimationFrame
...
- 会将每一帧中所有DOM操作集中一次渲染
- 重绘或回流的时间会随着浏览器的**刷新频率**动态改变，**不能主动设置**
...
框架对比我认为是开放问题，每个人都有不同见解，简单讲如：

R：用于构建用户界面的JavaScript库（只提供了UI层面解决方案）
V：渐进式JavaScript框架（致力于构建框架生态）

无论你自己的看法如何，首先都可以听听框架的作者的说法，比如 `AngularJS` 是 `Vue` 早期开发的灵感来源，`React` 则有更丰富的生态系统等，这些关于异同的观点都是出自 `Vue` 官方文档，尤其我们在面试中如果被问到照着说总不会出错。
...
> **推荐阅读**：[ 15张图，20分钟吃透Diff算法核心原理 ](https://juejin.cn/post/6994959998283907102#heading-8)（作者：Sunshine_Lin）
...
3. 图片懒加载、图片适当压缩
4. 不使用cookie、iframe、flash（要能说出缺点，iframe目前还有些特殊应用场景不可替代）
...
2. copy-webpack-plugin：搭配上面的使用，用于拷贝文件到编译目录
3. **外部模块CDN**：配置enternals可指定其引用域为window
4. **uglifyJsPlugin**：压缩混淆去注释
5. **异步加载**模块：即路由懒加载，把 import 放到函数中，这样执行时才会引用组件

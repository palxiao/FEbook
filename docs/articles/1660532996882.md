...
写这篇文章的目的，起初只是为了记录面试中**高频出现的问题**，逐渐汇集了 Web 前端中重要的知识点，与个人一些理解与浅析，在这个过程中发现弥补了自己不少疏漏的知识，所以分享出来交流学习，相信即使不是为了面试准备，也是值得一看的。内容**由浅入深**，非常适合 1 ~ 3 年经验前端阅读学习，3 ~ 5 年内开发查漏补缺。另外个人知识水平有限，如有错误还请不吝指正。
...
协商缓存就是服务器收到请求后进行更新判断，如果资源没有修改就返回 304 状态码（Not Modified）。
...
10. `height: 0`：根据盒模型原理，需要额外代码处理，还需设置 `overflow: hidden;` 隐藏子元素，注意还会被`display`影响
...
> **相关阅读**：[函数柯里化 - currying 实现代码](https://book.palxp.com/#/articles/1670497905099?id=%e5%87%bd%e6%95%b0%e6%9f%af%e9%87%8c%e5%8c%96-currying)
...
<details>
<summary> 参考下 chatGPT 如何回答~ </summary>
...
7. 使用 WebWorker 多线程来优化那些高耗时、计算密集型任务
8. Webpack 性能优化（可扩展，下文补充）
...
1. **DllPlugin**：把基础模块独立打包出来放到单独的动态链接库里
2. **CDN**：配置 **Enternals** 把一部分包分离出来在外部引用而不是打包进项目，然后可以储存在 CDN
3. **uglifyJsPlugin**：压缩混淆去注释插件
4. **image-webpack-loader**：对图片进行转化或者压缩处理
5. **tree shaking**：删除冗余代码（最早由 Rollup 提出，依赖于 ES6 模块化中的静态结构特性，也就是在编译时而非运行时就已经知道哪些模块被用到了）
6. **异步加载**模块：即路由懒加载，把 import 放到函数中，这样执行时才会引用组件

**从速度角度：**

1. 首先通过 **speed-measure-webpack-plugin** 插件测量各个 loader 花费的时间，量化打包速度，判断优化指标
1. **noParse** 忽略对部分没采用模块化的文件的递归解析处理
2. **babel-loader** 开启缓存，**cacheDirectory** 减少重新编译的消耗
3. 导入文件后缀要写全，使用 **alias 别名**设置路径而非相对路径（利用别名 Alisa 功能），避免查找尝试和递归路径的消耗。
4. **happypack** 多进程打包（不维护了，可以用 **thread-loader** 替代）

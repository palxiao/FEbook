金九银十送你一份前端详细知识手册，不敢说最全，但一定最实用

花了两个月时间整理的前端知识手册，本文内容由浅入深，大白话讲解，干货满满，适合各个阶段的前端人学习巩固，备战大厂。

> 正文中会出现的一些标记解释：
> 
> 「相关阅读」篇幅原因扩展内容写的小文章，或是笔者之前写过的相关文章
> 「推荐阅读」引用了别人的文章，或是官方权威文档
...
.self // 仅绑定元素自身触发
.once // 只渲染一次视图
.passive // 和移动端相关，滚动立即触发，不能和 .prevent 一起使用
```
...
为什么经典排序只说这两种，因为相对稳定性高，而冒泡是其中比较简单的一种排序手法，插入则优先于冒泡，虽然两者时间复杂度都是O(n²)，但是插入会比冒泡少一次赋值操作。

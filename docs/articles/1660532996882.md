...
其它的方式缺点太多建议了解即可，其中grid是最强大的二维布局方式，但只是做居中布局还是一维的flex就够用了。
...
> 1. 引用类型可能只有一种，那就是对象，在JS中万物皆对象，所以严格上讲数组应该算不上单独类型，可以说数组也属于对象。
> 
...
简单来说就是函数中嵌套函数（或函数中返回函数），作用是可以访问局部变量，缺点是容易发生内存泄漏（变量不会被自动回收）。
...
- 根据以上特性该方法常用于处理帧动画操作，而不是使用 `setInterval`

**requestIdleCallback**

- 回调属于**低优先级**任务，仅在浏览器空闲时期被调用（目前仍处于实验功能阶段，在微前端中常有应用）
...
无论你自己的看法如何，首先都应该听听框架的作者的说法，比如 `AngularJS` 是 `Vue` 早期开发的灵感来源，`React` 则有更丰富的生态系统等，这些关于异同的观点说辞都是出自 `Vue` 官方文档，尤其我们在面试中照着说总不会出错。

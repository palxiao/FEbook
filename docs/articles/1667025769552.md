...
在`color stops`中定义的颜色被放置在渐变射线上，从中心点开始，向右无限延伸。
...
这将我们带到了径向渐变语法的最后一部分：Color stops 颜色停止。
...
要做到这点有一种方法：找到从一个定位stop点到下一个stop点的范围，并在该范围内均匀分布非定位stop点，以此类推。

在我们的例子中，这意味着考虑红色到黄色的范围（从 50 到 100 像素），并找到蓝色的位置。因为那只是一个stop，所以把它放在范围的中心，75px。

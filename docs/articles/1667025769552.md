...
在这种情况下，结束形状与左侧还是右侧相交，具体取决于椭圆的水平半径的大小哪个最接近。并且椭圆的垂直半径的大小使得形状与顶部或底部相交，具体取决于哪个最接近。

在本例中，椭圆的水平半径的大小是这样的，以结束形状满足左侧或右侧，取决于哪一个是最近的。椭圆的垂直半径的大小是这样的，形状满足顶部或底部的边，取决于哪一个最近。
